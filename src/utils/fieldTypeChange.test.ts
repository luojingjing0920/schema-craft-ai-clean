...
describe("deriveFieldTypeChangePatch", () => {
    it("clears everything the target configuration cannot keep (select -> number)", () => {
        ...
        const patch = deriveFieldTypeChangePatch(field, "number", "text");
    ...
    it("clears rows but keeps placeholder and a compatible default (textarea -> text)", () => {
        ...
        const patch = deriveFieldTypeChangePatch(field, "string", "text");
    ...
    it("seeds options when a string becomes enum-backed without any", () => {
        ...
        expect(
            deriveFieldTypeChangePatch(field, "string", "select").options,
        ).toEqual(SEEDED_OPTIONS);
        expect(
            deriveFieldTypeChangePatch(field, "string", "radio").options,
        ).toEqual(SEEDED_OPTIONS);
    });
    ...
    it("keeps existing options when moving between enum-backed widgets", () => {
        ...
        const patch = deriveFieldTypeChangePatch(field, "string", "radio");
    ...
    it("does not seed string options for a boolean radio", () => {
        ...
        const patch = deriveFieldTypeChangePatch(field, "boolean", "radio");
    ...
    it("drops inline when the target widget is not radio", () => {
        ...
        const patch = deriveFieldTypeChangePatch(field, "string", "select");
...
describe("fieldTypeLabel", () => {
    it("keeps the five existing field kinds distinguishable", () => {
        expect(fieldTypeLabel({ dataType: "string", widget: "text" })).toBe(
            "string",
        );
        expect(fieldTypeLabel({ dataType: "string", widget: "textarea" })).toBe(
            "textarea",
        );
        expect(fieldTypeLabel({ dataType: "string", widget: "select" })).toBe(
            "select",
        );
        expect(fieldTypeLabel({ dataType: "number", widget: "text" })).toBe(
            "number",
        );
        expect(
            fieldTypeLabel({ dataType: "boolean", widget: "checkbox" }),
        ).toBe("boolean");
    });

    it("reports radios without collapsing them into their data type", () => {
        expect(fieldTypeLabel({ dataType: "string", widget: "radio" })).toBe(
            "radio",
        );
        // A boolean radio is still a boolean field.
        expect(fieldTypeLabel({ dataType: "boolean", widget: "radio" })).toBe(
            "boolean",
        );
    });
});
